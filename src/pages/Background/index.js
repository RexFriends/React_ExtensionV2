...
const scrapeProduct = (msg) => {
    fetch(APIURL + '/api/scrape-product?url=' + msg.URL)
        .then((res) => res.json())
        .then((json => {
            if (json.Success === true) {
                let product_info = json.product_info;
            } else {
                console.log('webscraper failed')
            }

        }).catch(() => console.log('scrape error', msg.URL)));
}